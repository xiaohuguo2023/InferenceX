...
SHIPPED_MLA_REF="${SHIPPED_MLA_REF:-/home/xiaohugu/work/vllm/vllm/v1/attention/backends/mla/rocm_aiter_mla.py}"
if [ "${RESEED_SHIPPED_MLA:-0}" = "1" ] && [ -f "$SHIPPED_MLA_REF" ]; then
...
REF = os.environ.get(
    "SHIPPED_MLA_REF",
    "/home/xiaohugu/work/vllm/vllm/v1/attention/backends/mla/rocm_aiter_mla.py",
)
s = open(F).read()
