#!/usr/bin/env python3
"""gpt-oss-120b fp4: MI355x (ours + IX dashboard) vs B200 vllm + B200 TRT.

Inputs
------
- IX dump JSONs at $IX_DUMP_DIR (default: /tmp/inferencex_dump/inferencex-dump-2026-04-27/)
- Our sweep dir as positional arg (or auto-detected latest /workspace/sweep_gptoss_*)

Outputs (in this script's directory)
------------------------------------
- gptoss120b_mi355x_vs_b200_tp{1,4,8}.png            2x3 panels per TP
- gptoss120b_pareto_{1k1k,8k1k}_tp{1,4,8}.png        interactivity Pareto
- gptoss120b_pareto_e2el_{1k1k,8k1k}_tp{1,4,8}.png   E2EL Pareto
- gptoss120b_data_table.csv                          per-combo numbers across all sources

Run
---
  python3 comparison_plots/plot_gptoss_compare.py [SWEEP_DIR]
"""
...
# gpt-oss-120b fp4 single-node (audited from inferencex-dump-2026-04-27):
#   MI355x vllm: 136 (TP=1), 153 (TP=4), 130 (TP=8)  — full grid, full CONC
#   B200 vllm:   150 (TP=1), 114 (TP=4), 158 (TP=8)  — full grid, full CONC
#   B200 trt:    131 (TP=1), 133 (TP=4), 139 (TP=8)  — full grid, TRT goes up to CONC=256 at TP=1/4
SOURCES = {
    "ours (MI355x widegraph-default)": {"kind": "sweep"},
    "IX MI355x vllm":  {"kind": "ix", "cfgs": [136, 153, 130]},
    "IX B200 vllm":    {"kind": "ix", "cfgs": [150, 114, 158]},
    "IX B200 trt":     {"kind": "ix", "cfgs": [131, 133, 139]},
}

# Each TP gets its own figure; TPs we plot (gptoss-120b fits in 1 GPU at fp4):
TPS = [1, 4, 8]
# Columns of the 2x3 panel: (ISL, OSL)
COLS = [(1024, 1024), (8192, 1024)]
# Concurrency x-axis order (TRT goes higher than vllm at TP=1)
...
def find_latest_sweep_dir() -> Path | None:
    candidates = sorted(glob.glob("/workspace/sweep_gptoss_*"))
    if not candidates:
        # Also check the v017_output drop-zone where prior gptoss sweeps landed.
        candidates = sorted(glob.glob(
            "/home/xiaohugu/work/sweep_v017_output/sweep_gptoss_*"
        ))
...
def load_sweep(sweep_dir: Path) -> dict[tuple, dict]:
    ...
    (``gptoss_widegraph_default_mi355x_isl…``) and the harness convention
    (``gptoss_fp4_mi355x_isl…``).
    """
    import re
    out = {}
...
def plot_tp(tp: int, sweep: dict, ix_data: dict[str, dict], out_path: Path):
    ...
    fig.suptitle(f"gpt-oss-120b fp4 — TP={tp}: ours vs InferenceX dashboard", fontsize=14)
    ...
    style = {
        ...
        "IX B200 trt":                     dict(color="tab:purple", marker="d", lw=1.5),
    }
...
def plot_pareto(tp: int, isl: int, osl: int, sweep: dict, ix_data: dict[str, dict],
    ...
    ax.set_title(f"gpt-oss-120b fp4 — ISL={isl}, OSL={osl}, TP={tp}\nPareto: {subtitle} (CONC sweep along each curve)")
...
def main():
    ...
    for tp in TPS:
        plot_tp(tp, sweep, ix_data, OUT_DIR / f"gptoss120b_mi355x_vs_b200_tp{tp}.png")
        for (isl, osl) in COLS:
            tag = f"{isl//1024}k{osl//1024}k"
            plot_pareto(tp, isl, osl, sweep, ix_data, "interactivity",
                        OUT_DIR / f"gptoss120b_pareto_{tag}_tp{tp}.png")
            plot_pareto(tp, isl, osl, sweep, ix_data, "e2el",
                        OUT_DIR / f"gptoss120b_pareto_e2el_{tag}_tp{tp}.png")
    write_csv(sweep, ix_data, OUT_DIR / "gptoss120b_data_table.csv")
